...
func _physics_process(delta):
	if is_round_running:
		...
		if stamina > 1 and not need_stamina_recharge:
			if Input.get_axis("null_axis" , "p2_break") >= 0.5:
				var break_strength = max_break_strength * Input.get_axis("null_axis" , "p2_break")
				velocity /= break_strength + 1
				stamina -= break_strength + 1
			if Input.get_axis("null_axis" , "p2_boost") >= 0.5:
				var boost_strength = max_boost_strength * Input.get_axis("null_axis" , "p2_boost")
